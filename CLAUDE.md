...
- **Pulling statements from the bank / "sync my accounts"** → run `finance sync --adapter simplefin --auto-import --commit --json`. This pulls new transactions from all SimpleFIN-linked accounts (Chase Sapphire, Chase Freedom, Ally Checking, Ally Savings), updates their balances, and imports the transactions in one step. For accounts not on SimpleFIN (Apple Card, Bilt Blue), the user drops CSVs in `transactions/inbox/` and runs `finance import <file> --account <name> --commit`. After syncing, report what was pulled (new transactions, duplicates skipped, balances updated). If the user just says "sync" without specifying, use the SimpleFIN adapter with auto-import and commit.

- **"Show me the dashboard" / "open the dashboard"** → run `finance dashboard`. This starts a local web server at `http://127.0.0.1:8765` with visual versions of every analytics feature. The dashboard is read-only and runs entirely locally.
